const client = contentful.createClient({
    ...
    accessToken: "gAfQphn4ymiJ3XzvoHwAwpKqY83JRPHpR73xdRJjUzk"
});
...
class UI {
    ...
    getBagButtons() {
        // we use the three dots to convert the list into an array
        const buttons = [...document.querySelectorAll('.bag-btn')];
        buttonsDOM = buttons;
        // looping through the buttons array to get the id attribute
        buttons.forEach(button => {
            let id = button.dataset.id;
            // checking if the item alreaddy exists in the cart
    ...
    setCartValues(cart) {
        let tempTotal = 0;
        let itemsTotal = 0;
        // we use .map() method to iterate through each item added to the cart to calculate tempTotal and itemsTotal
        cart.map(item => {
